	...
	if(len(cData) == 2):
		...
		for i in range(2):		
			for j in range(len(cData[i])):
				x_vector[i].append(np.concatenate((cData[i][j].eduStatus,cData[i][j].marryStatus, cData[i][j].occupation),0))
	else:
		x_vector = []
		y_vector = []
...
def splitData(data, seed ,ratio):
	...
	random.seed(seed)
	random.shuffle(data)
	train_data = data[:math.floor(dataLen*ratio)]
	valid_data = data[math.floor(dataLen*ratio):]
...
init_vector = [-0.01] * len(x_valid_vector[0])
